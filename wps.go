...
func loadYAML() (config wpsConfig, err error) {
...
func main() {
	...
	office6dir := filepath.Join(directory, "office6")
	...
	if dep {
		...
		for _, binary := range binaries {
			libraries = parseLibraries(binary, libraries, directory, binaries)
		}
...
func parseLibraries(file string, libraries []string, wpsDir string, selfBinaries []string) []string {
	out, _ := exec.Command("ldd", file).Output()

	for _, line := range strings.Split(string(out), "\n") {
		library := parseLibrary(line, wpsDir, selfBinaries)
		if len(library) > 0 {
			if ok, err := slice.Contains(libraries, library); !ok || err != nil {
...
func findDependencies(libraries []string) map[string]struct{} {
	packages := make(map[string]struct{})
	support64 := support64bit()
	client := proxyClient()
...
func openSUSEversion() (version string) {
	...
	pretty := "PRETTY_NAME=\"openSUSE "
	for scanner.Scan() {
		if strings.HasPrefix(scanner.Text(), pretty) {
			version = strings.ReplaceAll(strings.ReplaceAll(scanner.Text(), pretty, ""), "\"", "")
		}
	}
	return version
}

// support64bit if the system is a 64bit platform
func support64bit() bool {
	arch := runtime.GOARCH
